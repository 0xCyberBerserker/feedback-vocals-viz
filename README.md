...
- **Mic settings** (gear button on the highway) — input device picker (independent of
  ...
  a **Mic timing** slider (±1000 ms) that calibrates scoring for wireless audio/mic
  delay.
...
- A microphone. Any input the browser can see works; measurable capture latency is
  compensated automatically, and what can't be measured (Bluetooth audio delay,
  driver quirks) can be calibrated out with the **Mic timing** slider.

## Install

Two ways to install, with different update stories:

**From a release zip (simplest):** download the plugin zip from the releases page and
extract the `feedback-vocals-viz` folder into your FeedBack user-plugins directory.
Restart FeedBack. Updating later means re-downloading the newer zip and replacing the
folder by hand — a zip install has no `.git`, so it is **not** picked up by FeedBack's
in-app *Check for Updates*.

**From a git clone (updatable in-app):** clone this repository into the user-plugins
directory (or clone it elsewhere and junction/symlink it in). Because the folder is a
git checkout, FeedBack's plugin manager updates it in place via **Check for Updates →
Update** — a `git pull` that applies on the next restart. Keep it tracking the default
branch (`main`) as a clean checkout: the update is fast-forward-only, so it is skipped
if the checkout has local commits or uncommitted changes.
...
delay the plugin can't measure). It re-dates incoming mic frames — audio
playback and note/lyric drawing are untouched (display lag is what the
game-wide A/V offset is for). Because the sung trace is drawn from the same
frames, it shifts with the slider: sing along on time and adjust until your
trace sits on the note bars. Double-click the slider to reset to 0.

The `octaveIndependent`, `tolerance`, and `micOffsetMs` settings are also exposed
to splitscreen's per-panel controls via FeedBack's visualization settings contract.
